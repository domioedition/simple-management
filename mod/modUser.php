...
if($_GET['action'] == 'edit'){
	...
	if($_SESSION['userAccess']>=14){
		...
		<p>
			...
			<a href="?page=user&action=resetpass&id='.$id.'"><button type="button" class="btn btn-primary"  onclick="return confirm(\'Вы действительно хотите сбросить пароль?\');">Reset password</button></a>
...
if($_GET['action'] == 'resetpass'){
	...
	if(!empty($id)){
		...
		if (!$error){
			...
			$query = "UPDATE `users` SET `password`='{$hashed_password}', `salt`='{$salt}' WHERE `id`='{$id}'";
			if(mysqli_query($link, $query) or die(mysqli_error())){
				successMsg("Пароль успешно изменен. Новый пароль <b>\"123456\"</b>.");
			}
		}
		else{
			print '<h3>Возникли следующие ошибки</h3>' . $errort;
